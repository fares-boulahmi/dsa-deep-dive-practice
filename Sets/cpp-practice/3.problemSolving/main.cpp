...
int basicOps_problem1Solution(const vector<int>& nums) {
    ...
    {
        basicOps.insert(nums[i]);
    }
    return basicOps.size();
}
...
void basicOps_problem3Solution(set<int>& s) {
    ...
    {
        // cout << "the element is " << elm << endl;
       if ((elm % 2) == 0)
       {
        // cout << "the elemnt will erase is :" << elm << endl;
        // s.erase(elm);
        // cout << "after erase " << endl;
        delElm.push_back(elm);
       }
       
    }
    for (size_t i = 0; i < delElm.size(); i++)
    {
        s.erase(delElm[i]);
    }
